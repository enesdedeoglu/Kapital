    ...
    for (let i = 0; i < plan.count; i++, index++) {
      ...
      await sql`
        ...
        VALUES (${company!.id}::uuid, ${profile.archetype}, ${profile.riskTolerance},
                ...
                ${profile.strategyIntervalTicks})
        ON CONFLICT (company_id) DO NOTHING`;

      // Her NPC arketipinin tesis havuzundan bir tanesini alır; büyük
      // arketipler (üreticiler) ikinci bir tesis daha kurar.
      const slots = plan.facilities.length === 1 ? 1 : (rng() < 0.35 ? 2 : 1);
      for (let s = 0; s < slots; s++) {
        const [facilityCode, productCode] = plan.facilities[(index + s) % plan.facilities.length]!;
